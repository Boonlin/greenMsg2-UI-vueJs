    ...
    $.i18n.prop = function (key /* Add parameters as function arguments as necessary  */) {
        ...
        */
        ...
        if (typeof(value) == 'string') {
            ...
            var arr = [], j, index;
            i = 0;
            while (i < null2Void(value).length) {
                if (value.charAt(i) == '\'') {
                    // Handle quotes
                    if (i == null2Void(value).length - 1) {
                        ...
                        while ((j = value.indexOf('\'', j)) != -1) {
                            if (j == null2Void(value).length - 1 || value.charAt(j + 1) != '\'') {
        ...
        if (null2Void(value).length === 0) {
            return "";
        }
        if (null2Void(value).length == 1 && typeof(value[0]) == "string") {
            return value[0];
        }

        var str = "";
        for (i = 0, j = null2Void(value).length; i < j; i++) {
...
//            }
            ...
            if (line.length > 0 && line.match("^#") != "#") { // skip comments
                ...
                if (pair.length > 0) {
                    ...
                    while (value.search(/\\$/) != -1) {
                        value = value.substring(0, null2Void(value).length - 1);
    ...
    function null2Void(dat){
    	if(dat==undefined||typeof(dat)==undefined||dat==null||(typeof(dat)=="string"&&$.trim(dat)=="")){
    		return "";
    	}else{
    		return dat;
    	}
