	...
	jexTbl: function(cmd, opt) {
		...
		var jexTblFn={
						...
						addRow	:function(opt, i) {
							...
							if (opt!=null) { 
								$.each(tr.find("[id]"), function(i,v) {
									if ($(this).attr("id") == undefined||$(this).attr("id")=="") return true;
									var prt_dt = opt[$(this).attr("id")];
									
									if (typeof(prt_dt)!="string"&&prt_dt!=undefined) {
										prt_dt = JSON.stringify(prt_dt);
										prt_dt = prt_dt.replace(/\[\"/,"[").replace(/\"\]/,"]");
									}
								...
								if ($r.data("opt").onDblclick != null && $r.data("opt").onDblclick != undefined) {
...
//							if (s==1) s=0;
						...
						setPage	 :function(opt) {
							...
							var curPage		=$r.data("opt").curPage;
						...
						get		:function(opt) {
							if (typeof(opt) == "number") {
								...
								$.each($dat, function(i,v) { 
									result.push(_jexTbl_Get_Tr($(this)).data("dat")); 
								});
								return result;
							}
						},
